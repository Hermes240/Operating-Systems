...
int get2s(char *s)
{
	...
	while (c != EOF && c != '\r' && c != '\n') {
		*cp++ = c;
		if ((S_ISCHR(instat.st_mode)) && (S_ISCHR(outstat.st_mode)))
			mputc(c);
		if ((S_ISCHR(instat.st_mode)) && !(S_ISCHR(outstat.st_mode)))
			write(2, (char *)(&c), 1);
		c = getc();
...
int open(char *file, int flag)
{
	char buf[256];
	STAT curFile;
	if ((stat(file, &curFile) == 0) && ((curFile.st_mode & 0xF000) == 0xA000))
	{
		if (readlink(file, buf) != -1)
		{
			open(buf, flag);
		}
		else
		{
			dup2(1, 4); dup2(2, 1); printf("open: %s: could not read link\n", file); dup2(4, 1);
		}
	}
	else
	{
...
//loads the basename from the filepath into buf
void getBasename(char *buf, char *filepath)
{
	int i = 0;
	int j = -1;

	//getting the file name from the path string...
	while (filepath[i])
	{
		if (filepath[i] == '/') { j = i; }
		i++;
...
//load the the path of the parent from filepath into buf
void getParentpath(char *buf, char *filepath)
{
	int i = 0;
	int j = -1;
